...
from DestinyBot.confing import get_int_key, get_str_key
from DestinyBot.services.telethonbasics import is_admin
from DestinyBot.events import register
from pymongo import MongoClient
from DestinyBot.modules.sql.nsfw_watch_sql import (
    add_nsfwatch,
    get_all_nsfw_enabled_chat,
    is_nsfwatch_indb,
    rmnsfwatch,
)
from DestinyBot import telethn as tbot, MONGO_DB_URI, BOT_ID
        ...
        if event.photo:
            if event.chat_id == c["id"]:
                ...
                if nude.is_nude("./nudes.jpg"):
                    ...
                    st = sender.first_name
                    hh = sender.id
                    final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, Destiny deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By DestinyAI` \n**#GROUP_GUARDIAN** "
                    dev = await event.respond(final)
                    await asyncio.sleep(10)
                    await dev.delete()
                    os.remove("nudes.jpg")
